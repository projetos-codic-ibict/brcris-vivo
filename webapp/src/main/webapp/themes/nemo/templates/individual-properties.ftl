		 ...
		 <div class="panel panel-default"> 
			   <div class="panel-heading">
				...
				<#if rangeClass == "Authorship" && individual.editable && (property.domainUri)?? && property.domainUri?contains("Person")>
					<h3 test="jesiel" id="${property.localName}-${rangeClass}" title="${property.publicDescription!}" class="panel-title">${property.name} <@p.addLink property editable /> <@p.verboseDisplay property /> 
				...
				<#elseif rangeClass == "Name" && property.statements?has_content && editable >
					<h3 test="jesiel" id="${property.localName}" 
				...
				<#elseif rangeClass == "Authorship" && !individual.editable && (property.domainUri)?? && property.domainUri?contains("Person")>
					<h3 test="jesiel" id="${property.localName}-${rangeClass}" 
						title="${property.publicDescription!}" 
				...
				<div class="panel-body">
					<div test="jesiel" class="property-list" style="width:100%;" role="list" id="${property.localName}-${rangeClass}-List" displayLimit="${limit}">
					<#-- data property -->
